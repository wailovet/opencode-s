// 复刻自 @/components/prompt-input/submit.ts — 修改：navigate 到 /vscode-session 而不是 /session
...
export function createVSCodePromptSubmit(input: PromptSubmitInput) {
  ...
  const handleSubmit = async (event: Event) => {
    ...
    if (!session && isNewSession) {
      ...
      if (created) {
        ...
        else navigate(`/${base64Encode(sessionDirectory)}/vscode-session/${session.id}`)
      }
    }
    if (!session) {
      showToast({
