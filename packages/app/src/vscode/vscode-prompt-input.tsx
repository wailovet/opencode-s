...
// VSCode patch: 使用本地复刻的 submit，导航到 /vscode-session 而非 /session
import { createVSCodePromptSubmit, type FollowupDraft } from "./vscode-submit"
...
export const VSCodePromptInput: Component<PromptInputProps> = (props) => {
  ...
  const { abort, handleSubmit } = createVSCodePromptSubmit({
    info,
    imageAttachments,
    commentCount,
  ...
  const selectProject = (worktree: string) => {
    ...
    navigate(`/${base64Encode(worktree)}/vscode-session`)
  }
  const addProject = () => {
